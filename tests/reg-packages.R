...
stopifnot(exprs = {
    ...
    ## PR#18960: checkRd() on package Rd files from outside package dir
    inherits(tools::checkRd("myTst/man/myTst-package.Rd", stages = "build"), "checkRd")
})
...
## PR#18960: .check_package_parseRd() does not alter _R_RD_MACROS_PACKAGE_DIR_
## and checkRd() on exSexpr/man/foo.Rd with \packageAuthor succeeds directly
stopifnot(exprs = {
    identical(Sys.getenv("_R_RD_MACROS_PACKAGE_DIR_", ""), "")
    length(tools::checkRd(file.path(pkgPath, "exSexpr", "man", "foo.Rd"),
                          stages = c("build", "install"))) == 0L
})
