...
local({
  ...
  writeLines(c(
      ...
      "setMethod('Math', 'foo', function(x) x)"
  ), tmp)
  package.skeleton("myTstS4", code_files=tmp)
  on.exit(unlink("myTstS4", recursive=TRUE), add=TRUE)

  # package.skeleton automatically adds Imports: methods to DESCRIPTION in newer R.
  # But we must ensure it imports methods in NAMESPACE for minimal session.
  writeLines(c("import(methods)", readLines("myTstS4/NAMESPACE")), "myTstS4/NAMESPACE")

  build.pkg("myTstS4")
  on.exit(unlink(Sys.glob("myTstS4_*")), add=TRUE)

  install.packages("myTstS4", lib="myLib", repos=NULL, type="source")
  on.exit(unlink("myLib/myTstS4", recursive=TRUE), add=TRUE)
  ...
  if (!is.null(status) && status != 0) {
      cat("Failed to unload myTstS4 in minimal session:\n")
      cat(res, sep = "\n")
      stop("regression test failed")
  }
})

## Test that a custom S4 generic representing a base function (like 'nrow')
## IS successfully uncached when the package defining it is unloaded,
## addressing the concern that it might remain generic forever.
local({
  tmp <- tempfile(fileext = ".R")
  on.exit(unlink(tmp))
  writeLines(c(
      "setClass('foo', contains='numeric')",
      "setGeneric('nrow')",
      "setMethod('nrow', 'foo', function(x) 42)"
  ), tmp)
  if(file.exists("myTstNrow")) unlink("myTstNrow", recursive=TRUE)
  package.skeleton("myTstNrow", code_files=tmp)
  on.exit(unlink("myTstNrow", recursive=TRUE), add=TRUE)

  writeLines(c("import(methods)", "exportClasses(foo)", "export(nrow)"), "myTstNrow/NAMESPACE")

  build.pkg("myTstNrow")
  on.exit(unlink(Sys.glob("myTstNrow_*")), add=TRUE)

  install.packages("myTstNrow", lib="myLib", repos=NULL, type="source")
  on.exit(unlink("myLib/myTstNrow", recursive=TRUE), add=TRUE)

  # Run in minimal session and verify it is NOT generic after unload
  Rscript <- file.path(R.home("bin"), "Rscript")
  cmd <- paste(
      "library(methods);",
      "loadNamespace('myTstNrow', lib.loc='myLib');",
      "stopifnot(isGeneric('nrow'));",
      "unloadNamespace('myTstNrow');",
      "stopifnot(!isGeneric('nrow'))"
  )
  ...
  status <- attr(res, "status")
  if (!is.null(status) && status != 0) {
      cat("Failed to verify custom base generic uncaching in minimal session:\n")
      cat(res, sep = "\n")
      stop("regression test failed")
  }
})
