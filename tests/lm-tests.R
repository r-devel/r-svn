...
stopifnot(exprs = {
    all.equal(residuals(roller.glm0, type = "partial"),
              residuals(roller.lm0,  type = "partial"), tolerance = 1e-14)  # 4.0e-16
    all.equal(deviance(roller.lm),
              deviance(roller.glm), tolerance = 1e-14) # 2.4e-16
    all.equal(weighted.residuals(roller.lm),
              residuals         (roller.glm), tolerance = 2e-14) # 9.17e-16
    all.equal(deviance(roller.lm0),
              deviance(roller.glm0), tolerance = 1e-14) # 2.78e-16
    all.equal(weighted.residuals(roller.lm0, drop0=FALSE),
              residuals         (roller.glm0),  tolerance = 2e-14) # 6.378e-16
})
...
stopifnot(exprs = {
    ...
    all.equal(print(anova(roller.lm0), signif.stars=FALSE),
