...
##
## Numeric 'bytes' elements are stored in NATIVE byte order, so that
...
## bytesRaw() and as.bytes(<raw>) are layout-dependent, by design.
##
...
##
## Two ways to run it:
##   build/bin/Rscript tests/bytesxp-dev/endcheck.R        # this platform
## and, to exercise the other arm of BYTEVEC_MSB without the hardware,
## rebuild with -DR_BYTES_SIMULATE_BIGENDIAN and set
## R_BYTES_GENERIC_ARITH=1 (the native kernels reinterpret an element's
...
SPECS <- list(list(1L, "unsigned"), list(2L, "signed"), list(4L, "unsigned"),
              ...
              list(16L, "unsigned"), list(16L, "signed"), list(21L, "signed"))
...
for (spec in SPECS) {
    ...
    x <- suppressWarnings(as.bytes(txt, w, k))
    keep <- !is.na(x)
    x <- x[keep]; txt <- txt[keep]
    ok("some values fit", length(x) > 3L)

    ## 1. text is the inverse of parsing, whatever the layout
    ok("as.character inverts as.bytes", identical(as.character(x), txt))
    ok("as.bytes inverts as.character",
       identical(as.bytes(as.character(x), w, k), x))
    ...
    xn <- as.bytes(c(txt[1], NA, txt[2]), w, k)
    ...
    if (w %in% c(1L, 2L, 4L, 8L, 16L)) {
        small <- suppressWarnings(as.bytes(c("3", "7", "10"), w, k))
    ...
    ok("as.numeric of a small value",
       identical(as.numeric(as.bytes("42", w, k)), 42))
...
cat("\n-- opaque kind (layout is the value; must be verbatim) --\n")
h <- "0102030405060708090a0b0c0d0e0f10"
op <- as.bytes(h, 16L, "opaque")
ok("hex text round trips", identical(as.character(op), h))
ok("payload is verbatim, not reordered",
   identical(bytesRaw(op), as.raw(c(1:15, 16))))
ok("as.bytes(raw) is the inverse", identical(as.bytes(bytesRaw(op), 16L), op))
f <- tempfile(fileext = ".rds")
suppressMessages(saveRDS(op, f))
ok("opaque serialization round trip", identical(readRDS(f), op))
unlink(f)
