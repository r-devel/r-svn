...
static int StringValue(int c, Rboolean forSymbol)
{
	...
	if ((unsigned int) c < 0x80) WTEXT_PUSH(c);
	    ...
	    /* have an 8-bit char in the current encoding; FIX ME: `wc` values
	       will be wrong when native encoding differs from that indicated
	       `known_to_be*` globals set by `do_parse`. */
	    int res = 0;
