 ...
 *
 * Assumptions:
 * * Buffer length > min + R_MB_CUR_MAX, i.e.  we'll find a complete character
 *   before we run out of buffer.
 * * MBCS encodings are valid (they've been read through once already so they
 *   should be).
 * * Stateless encodings.
 */
...
static int skipBytesByChar(char * c, int min) {
	...
	if(utf8locale) {
	    /* Find first non continuation byte; we assume UTF8 is valid */
	    char * cc = c + min;
...
#define CTEXT_PUSH(c) do {                                             \
	if (ct - currtext >= 1000) {                                   \
	    int shift = skipBytesByChar(currtext, 100 + 4);            \
	    memmove(currtext, "... ", 4);                              \
	    memmove(currtext + 4, currtext + shift, 1000 - shift + 1); \
	    ct -= shift - 4;                                           \
