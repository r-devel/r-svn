...
/* The dense rank of a 'bytes' key: equal elements get equal integers,
...
static SEXP bytesDenseRank(SEXP x)
{
	...
	if (prev == NULL || R_bytesEltCmp(p, prev, w, k) != 0)
	    rank++;
	r[j] = rank;
	prev = p;
    }
...
attribute_hidden SEXP do_radixsort(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    */
    ...
    for (SEXP ap = (nl <= INT_MAX) ? args : R_NilValue; ap != R_NilValue;
	 ap = CDR(ap))
	if (TYPEOF(CAR(ap)) == BYTESXP)
	    SETCAR(ap, bytesDenseRank(CAR(ap)));
