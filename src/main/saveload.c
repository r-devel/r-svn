...
attribute_hidden SEXP do_savefile(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    /* Settle the version while the old file is still intact.  Both the
       object walk for a defaulted version and rejecting an explicit one
       can error; doing either after fopen("wb") would already have
       truncated the destination. */
    if (version == 0)
	version = R_SerializeVersionFor(CAR(args), defaultSaveVersion());
    else
