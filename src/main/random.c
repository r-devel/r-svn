...
attribute_hidden SEXP do_random1(SEXP call, SEXP op, SEXP args, SEXP rho)
{
	...
	!(isNumeric(CADR(args)) || TYPEOF(CADR(args)) == XINTSXP))
	invalid(call);
...
attribute_hidden SEXP do_random2(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    if (!isVector(CAR(args)) ||
	!(isNumeric(CADR(args)) || TYPEOF(CADR(args)) == XINTSXP) ||
	!(isNumeric(CADDR(args)) || TYPEOF(CADDR(args)) == XINTSXP))
	invalid(call);
    if (XLENGTH(CAR(args)) == 1) {
#ifdef LONG_VECTOR_SUPPORT
	double dn = asReal(CAR(args));
...
attribute_hidden SEXP do_random3(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    if (!(isNumeric(a) || TYPEOF(a) == XINTSXP) ||
	!(isNumeric(b) || TYPEOF(b) == XINTSXP) ||
	!(isNumeric(c) || TYPEOF(c) == XINTSXP))
	invalid(call);
    na = XLENGTH(a);
    nb = XLENGTH(b);
    nc = XLENGTH(c);
