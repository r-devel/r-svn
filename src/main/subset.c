...
attribute_hidden SEXP ExtractSubset(SEXP x, SEXP indx, SEXP call)
{
    ...
    case XINTSXP:
	{
	    ...
	    EXTRACT_SUBSET_LOOP(memcpy(XINT_ELT(result, i),
				       XINT_ELT_RO(x, ii), w),
				(R_xintCheckNA(result),
				 R_xintSetEltNA(XINT_ELT(result, i),
						 (int) w, k)));
	}
	break;
    case LISTSXP:
...
static SEXP MatrixSubset(SEXP x, SEXP s, SEXP call, int drop)
{
    ...
    case XINTSXP:
	{
	    size_t w = (size_t) XINT_WIDTH(x);
	    int k = XINT_KIND(x);
	    MATRIX_SUBSET_LOOP(memcpy(XINT_ELT(result, ij),
				      XINT_ELT_RO(x, iijj), w),
			       (R_xintCheckNA(result),
				R_xintSetEltNA(XINT_ELT(result, ij),
						(int) w, k)));
	}
	break;
    default:
...
static SEXP ArraySubset(SEXP x, SEXP s, SEXP call, int drop)
{
    ...
    case XINTSXP:
	{
	    ...
	    ARRAY_SUBSET_LOOP(memcpy(XINT_ELT(result, i),
				     XINT_ELT_RO(x, ii), w),
			      (R_xintCheckNA(result),
			       R_xintSetEltNA(XINT_ELT(result, i),
					       (int) w, kind)));
