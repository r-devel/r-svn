...
AnswerType(SEXP x, bool recurse, bool usenames, struct BindData *data, SEXP call)
{
	...
	if (data->ans_width) {
	    ...
	    else if (data->ans_width != XINT_WIDTH(x)) {
		data->ans_clash = 1;
		data->ans_clashwidth = XINT_WIDTH(x);
	    }
	    else if (data->ans_kind != XINT_KIND(x))
		data->ans_clash = 2;
	    else if (data->ans_nona != !XINT_HAS_NA(x))
		data->ans_clash = 3;
	}
	else {
	    data->ans_nona = !XINT_HAS_NA(x);
	    data->ans_width = XINT_WIDTH(x);
	    data->ans_kind = XINT_KIND(x);
	}
	data->ans_flags |= 1024;
	data->ans_length += XLENGTH(x);
	break;
...
{
    /* 'xinteger' (1024) is not a step in the coercion chain below but a
	...
	/* the answer is an 'xinteger' vector, so now the arguments do have
	   to agree on which one; see AnswerType() */
	switch (data->ans_clash) {
	case 1:
	    errorcall(call, _("cannot combine 'xinteger' vectors of widths %d and %d"),
		      data->ans_width, data->ans_clashwidth);
	case 2:
	    errorcall(call, _("cannot combine 'xinteger' vectors of different kinds"));
	case 3:
	    errorcall(call,
		      _("cannot combine 'xinteger' vectors that differ in whether NA is representable"));
	}
	/* logical (2) and integer (16) narrow into 'xinteger'. */
...
/* allocVector() cannot carry the per-vector width and kind an 'xinteger'
...
ListAnswer(SEXP x, int recurse, struct BindData *data, SEXP call)
{
	...
	for (i = 0; i < XLENGTH(x); i++) {
	    ...
	    memcpy(XINT_DATA(e), XINT_ELT_RO(x, i),
		   (size_t) XINT_WIDTH(x));
	    LIST_ASSIGN(e);
	    UNPROTECT(1);
	}
...
static void
XIntAnswer(SEXP x, struct BindData *data, SEXP call)
{
    R_xlen_t i;
    switch(TYPEOF(x)) {
    case NILSXP:
    ...
    case XINTSXP:
	{
	    ...
	    R_xintMemcpy(XINT_ELT(data->ans_ptr, data->ans_length),
			  XINT_DATA_RO(x),
			  (size_t) nx * XINT_WIDTH(x));
	    data->ans_length += nx;
	}
	break;
    case LGLSXP:
    case INTSXP:
	{
	    ...
	    SEXP nx = PROTECT(R_xintNarrow(x, XINT_WIDTH(ap),
					    XINT_KIND(ap),
					    XINT_HAS_NA(ap), call));
	    R_xlen_t nn = XLENGTH(nx);
	    R_xintMemcpy(XINT_ELT(ap, data->ans_length),
			  XINT_DATA_RO(nx),
			  (size_t) nn * XINT_WIDTH(ap));
	    data->ans_length += nn;
	    UNPROTECT(1);
	}
	break;
		  ...
		  int deparse_level)
{
	...
	for (t = args; t != R_NilValue; t = CDR(t)) {
		...
		/* NULL contributes nothing, as coerceVector() makes it
		   raw(0) in the branch above; R_xintNarrow() is the
		...
		R_xlen_t k = XLENGTH(u);
		R_xlen_t idx = (!umatrix) ? rows : k;
		R_xintCopyWithRecycle(result, u, n, idx, k);
		n += idx;
		UNPROTECT(1);
	    }
	}
		  ...
		  int deparse_level)
{
	...
	for (t = args; t != R_NilValue; t = CDR(t)) {
		...
		R_xintFillMatrixWithRecycle(result, u, n, rows, idx, cols, k);
		n += idx;
		UNPROTECT(1);
	    }
	}
