...
static void extractItem(char *buffer, SEXP ans, R_xlen_t i, LocalData *d)
{
    ...
    case ALTSXP:
	{
	    ...
	    /* Strtoi gives NA_INTEGER for a value out of int range as
	       ...
	       NA-with-a-warning that as.xinteger() gives. */
	    else {
		R_xint_parse_t st = R_xintEltFromString(p, buffer, w, k,
						       XINT_HAS_NA(ans));
		if (st == XINT_PARSE_OK && XINT_HAS_NA(ans) &&
		    R_xintEltIsNA(p, w, k))
		    d->xintReserved++;
		else if (st != XINT_PARSE_OK) {
		    char what[32];
		    snprintf(what, sizeof what, "%s", R_xintTypeName(ans));
...
/* Takes the prototype rather than its SEXPTYPE: an 'xinteger' vector's
...
attribute_hidden SEXP do_scan(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    PROTECT(ans);
    endcontext(&cntxt);
    R_xintWarnReservedCount(data.xintReserved);
