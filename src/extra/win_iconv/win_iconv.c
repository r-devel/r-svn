...
iconv_t iconv_open(const char *tocode, const char *fromcode)
{
    ...
    if (win_iconv_open(cd, tocode, fromcode))
    {
        ...
        if (valstr)
        {
            if (!stricmp(valstr, "true") || !stricmp(valstr, "yes") || !strcmp(valstr, "1"))
                R_WIN_ICONV_best_fit = 1;
            else if (!stricmp(valstr, "false") || !stricmp(valstr, "no") || !strcmp(valstr, "0"))
...
static int kernel_wctomb(csconv_t *cv, ushort *wbuf, int wbufsize, uchar *buf, int bufsize)
{
    ...
    if (!must_use_null_useddefaultchar(cv->codepage))
    {
        ...
        /* http://msdn.microsoft.com/en-us/library/dd374130%28VS.85%29.aspx
           ...
           Windows 2000.
         */
        if (!(cv->flags & FLAG_TRANSLIT) && (cv->codepage == 20127 || R_WIN_ICONV_best_fit == 0))
