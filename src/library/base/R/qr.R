...
qr.default <- function(x, tol = 1e-07, LAPACK = FALSE, ...)
{
    ...
    ## else "Linpack" case:
    p <- as.integer(ncol(x))
    if(is.na(p)) stop(gettextf("invalid %s", "ncol(x)"), domain=NA)
    n <- as.integer(nrow(x))
...
qr.coef <- function(qr, y)
{
    ...
    n <- as.integer(nrow(qr$qr)); if(is.na(n)) stop(gettextf("invalid %s", "nrow(qr$qr)"), domain=NA)
    p <- as.integer(ncol(qr$qr)); if(is.na(p)) stop(gettextf("invalid %s", "ncol(qr$qr)"), domain=NA)
    k <- as.integer(qr$rank);     if(is.na(k)) stop(gettextf("invalid %s", "ncol(qr$rank)"), domain=NA)
    im <- is.matrix(y)
    if (!im) y <- as.matrix(y)
    ny <- as.integer(ncol(y))
    if(is.na(ny)) stop(gettextf("invalid %s", "ncol(y)"), domain=NA)
...
qr.qy <- function(qr, y)
{
    ...
    if(is.na(n)) stop(gettextf("invalid %s", "nrow(qr$qr)"), domain=NA)
		  ...
		  ncol = if (complete) nrow(R) else min(dim(R)))
{
    ...
    if(is.na(p)) stop(gettextf("invalid %s", "NCOL(R)"), domain=NA)
    if (ncol < p)
