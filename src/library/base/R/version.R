...
.make_numeric_version <-
  function(x, strict = TRUE, regexp, classes = NULL) {
    ...
    if (!length(x)) {
      ...
      if (!is.character(x)) {
        msg <- gettextf(
          "invalid non-character version specification 'x' (type: %s)",
          typeof(x)
        )
        stop(msg, domain = NA)
      }
      ...
      if (strict && !all(i <- (ok | is.na(x)))) {
        stop(
          gettextf(
            "invalid version specification %s",
...
as.numeric_version <-
  function(x) {
    if (is.numeric_version(x)) {
      ...
      bad <- vapply(x, function(e) anyNA(e) || any(e < 0L), NA)
      if (any(bad)) {
...
.encode_numeric_version <-
  function(x) {
    ...
    structure(
      ifelse(lens > 0L, y, NA_character_),
      width = width,
      lens = lens,
      .classes = classes,
      names = nms
...
`[<-.numeric_version` <-
  function(x, i, j, value) {
    ...
    if (missing(j)) {
      ...
      if (any(vapply(value, function(e) anyNA(e) || any(e < 0L), NA))) {
        stop("invalid 'value'")
      }
      ## Listify j as needed.
      if (!is.list(j)) {
  ...
  function(x, ..., value) {
    ...
    if (nargs() < 4L) {
      if (length(..1) < 2L) {
        ...
        } else if (!is.integer(value) || anyNA(value) || (any(value) < 0L)) {
          stop("invalid 'value'")
        }
      } else {
...
Ops.numeric_version <-
  function(e1, e2) {
    if (nargs() == 1L) {
      stop(
        gettextf(
          "unary '%s' not defined for \"numeric_version\" objects",
          .Generic
        ),
        domain = NA
      )
    ...
    if (!boolean) {
      stop(
        gettextf("'%s' not defined for \"numeric_version\" objects", .Generic),
        domain = NA
      )
    }
