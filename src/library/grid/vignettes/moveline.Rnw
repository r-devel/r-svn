...
Many graphics systems maintain a concept of a current drawing location.
This concept has been incorporated into Grid, with the additional
advantage that the drawing location may be specified relative to an
arbitrary viewport. This capability facilitates drawing across
different coordinate systems\footnote{An early request on R-help
  specifically asked for such functionality, and subsequent requests
  have reiterated the need (e.g., from Ross Darnell concerning
  problems with segments and multiple graphs).}.

The following code presents a straightforward example (the resulting
