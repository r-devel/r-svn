...
predefined Grid graphics functions are relatively complicated\footnote{Although there are exceptions; some
...
without forcing them to use complicated concepts or write
complex code unless necessary. Along similar lines,
...
sophisticated form if required.

With the predefined graphics functions being fully developed and
complex implementations, there is a lack of examples of simple,
...
going from the simplest prototype implementation to the most
sophisticated. It demonstrates that if you only want simple graphics
output then you can do it quickly. It also demonstrates how to write
functions that allow your graphics to be used by other people, and how
to make your graphics interactive (as far as Grid allows).

@

This document should be read {\em after} the \grid{} Users'
Guide. Here we assume that the reader has an understanding
...
producing standard R graphical output. You issue a series of
...
plotting functions. This is called \emph{procedural graphics}.
...
data and margins around the outside for the axes to fit in, with a space
...
\code{lattice} plot is a good example. In such cases, it is more
...
\item We can generalise the plot. In this case, we can use it for
...
that \code{upViewport(0)} is required to get right back to the
top level).
...
output which is very flexible, can be annotated in arbitrary ways,
...
possible, for example, to change the \code{x} and \code{y} data
...
exist, but they are not bound together as a whole. When these
...
a \grob{} that can have other \grob{}s as children. Here is an example
for creating an \code{splot} \grob{}. Bits of the
construction are placed into separate functions, for reasons which will
...
\item The \code{name} argument allows a string identifier to be
  ...
  after drawing it and/or when it is part of a larger \grob{}.
...
\item The \code{children} argument provides a list of \grob{}s which
  ...
  produce \grob{}s and do not perform any drawing. Also notice that
  each child has a name; this makes it possible to access the
  components of the scatterplot.
...
\item The \code{childrenvp} argument provides a viewport (or
  ...
  In this way, viewports remain available for further annotation.
...
\item The \code{cl} argument means that the \grob{} created is a
  ...
  write methods specifically for our scatterplot.
\end{enumerate}
...
The following describes an \code{editDetails()} method for the
...
\grob{}s. Here is a simple scatterplot matrix \grob{}\footnote{{\bf
  ...
  \gTree{}, and wait for improvements in performance.}.
...
drawn via an API (rather than editing the original code). In
...
\I{M.~Kondrin}'s \pkg{Rgrace} package.
