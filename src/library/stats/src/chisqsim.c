...
static void chisqsim(int nrow, int ncol, const int nrowt[], const int ncolt[], int n, int B, const double expected[],
                     // modified :
                     int *observed, double *fact, int *jwork, double *results)
{
    ...
    for (int iter = 0; iter < B; ++iter)
    {
        ...
        for (int j = 0; j < ncol; ++j)
        {
            ...
            {
                double e = expected[ii], o = observed[ii];
                chisq += (o - e) * (o - e) / e;
            }
        }
        results[iter] = chisq;
...
static void fisher_sim(int nrow, int ncol, const int nrowt[], const int ncolt[], int n, int B,
                       // modified :
                       int *observed, double *fact, int *jwork, double *results)
{
    /* Calculate log-factorials.  fact[i] = lgamma(i+1) */
