...
static void Smirnov_sim_wrk(int nrow, int ncol, const int nrowt[], const int ncolt[], int n, int B, int *observed,
                            int twosided, double *fact, int *jwork, double *results)
{
...
SEXP Smirnov_sim(SEXP sr, SEXP sc, SEXP sB, SEXP twosided)
{
    ...
    Smirnov_sim_wrk(nr, nc, isr, INTEGER(sc), n, B, observed, INTEGER(twosided)[0], fact, jwork, REAL(ans));
    UNPROTECT(3);
    return ans;
}
